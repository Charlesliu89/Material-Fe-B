...
TOOLS_DIR = CALPHAD_DIR / "tools"
SOURCES_DIR = CALPHAD_DIR / "sources"
EXTRACTED_DIR = CALPHAD_DIR / "extracted"
TDB_PATH = CALPHAD_DIR / "tdb" / "Fe-B.tdb"
...
def main() -> int:
    ...
    extracted_path = EXTRACTED_DIR / "tdb_lines.jsonl"
    ...
    for iteration in range(args.max_iter):
        ...
        if args.candidates:
            ...
            if result.returncode != 0:
                print(result.stderr)
                if (SOURCES_DIR / "search_blocked.md").exists():
                    print(f"ERROR: manual search required (see {SOURCES_DIR / 'search_blocked.md'})")
                    return 2
        ...
        fetch_cmd = [
            sys.executable,
            str(TOOLS_DIR / "fetch_sources.py"),
            *dois,
            "--out",
            str(SOURCES_DIR),
        ]
        ...
        if fetch.returncode != 0:
            print(fetch.stderr)
            if (SOURCES_DIR / "download_queue.md").exists():
                print(f"ERROR: manual download required (see {SOURCES_DIR / 'download_queue.md'})")
                return 2

        pdf_cmd = [sys.executable, str(TOOLS_DIR / "pdf_to_text.py"), str(SOURCES_DIR)]
        ...
        txt_inputs = [str(SOURCES_DIR)]
        extract_cmd = [
            sys.executable,
            str(TOOLS_DIR / "extract_tdb_lines.py"),
            *txt_inputs,
            "--output",
            str(extracted_path),
        ]
        ...
        smoke_cmd = [sys.executable, str(TOOLS_DIR / "tdb_smoke_test.py"), str(TDB_PATH)]
