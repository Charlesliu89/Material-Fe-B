...
def _write_blocked(path: str, message: str) -> None:
    content = (
        ...
        f"{message}\n\n"
        "Suggestion: run tools/lit_search.py locally with outbound access and set "
...
def main() -> int:
    ...
    if not results:
        ...
        _write_blocked(
            "sources/search_blocked.md",
            f"Both Crossref/OpenAlex searches failed or were blocked. Details: {blocked_msgs}",
        )
        print("ERROR: search blocked; see sources/search_blocked.md")
        return 2
